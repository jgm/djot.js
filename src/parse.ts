...
const parse = function(input: string, options: ParseOptions = {}, currentFile?: string): Doc {
  ...
  const handlers: Record<string, (suffixes: string[],
	...
	const lab = normalizeLabel(node.data.key);
      ...
      /*file_inclusion: (suffixes, startpos, endpos, pos, data?: { altText?: string, filePath?: string }) => {
        if (data && data.altText && data.filePath) {
          const { altText, filePath } = data;
          if (options.includedFiles?.includes(filePath)) {
            const cycle = [...options.includedFiles, filePath].join(' -> ');
            throw new Error(`Cyclic inclusion detected: ${cycle}`);
          }
  
          const fileContent = readFileSync(filePath, { encoding: 'utf-8' });
          const newOptions = { ...options, includedFiles: [...(options.includedFiles || []), filePath] };
          const includedDoc = parse(fileContent, newOptions); // Recursively parse the included file
          includedDoc.children.forEach(child => {
            addChildToTip(child); // Add the parsed content to the current document
          });
        }
      },*/
      file_inclusion: (suffixes, startpos, endpos, pos, data?: { altText?: string, filePath?: string }) => {
