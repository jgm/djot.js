...
import { parse } from './parse'; // Import the parse function
...
type OpenerMap = { [opener: string]: Opener[] } // in reverse order
...
const findSpecial = function (s: string, startpos: number, endpos: number): number | null {
...
const pattFileInclusion = pattern("\\+\\[([^\\]]*)\\]\\(([^\\)]+)\\)");
const pattImageInclusion = pattern("!\\[([^\\]]*)\\]\\(([^\\)]+)\\)");
const matchers: { [key: number]: (self: InlineParser, pos: number, endpos: number) => number | null } = {};
...
matchers[C_PLUS] = function (self: InlineParser, pos: number, endpos: number): number | null {
  const subject = self.subject;
  const m = find(subject, pattFileInclusion, pos, endpos);
  if (m) {
    const altText = m.captures[0];
    const filePath = m.captures[1];
    self.addMatch(m.startpos, m.endpos, "file_inclusion", undefined, { altText, filePath });
    return m.endpos + 1;
  }
  return betweenMatched("+", "insert", "str", hasBrace)(self, pos, endpos);
};

matchers[C_BANG] = function (self: InlineParser, pos: number, endpos: number): number | null {
  const subject = self.subject;
  const m = find(subject, pattImageInclusion, pos, endpos);
  if (m) {
    const altText = m.captures[0];
    const filePath = m.captures[1];
    self.addMatch(m.startpos, m.endpos, "file_inclusion", undefined, { altText, filePath });
  ...
  return betweenMatched("!", "image_marker", "str", hasBrace)(self, pos, endpos);
};

matchers[C_BACKTICK] = function (self: InlineParser, pos: number, endpos: number): number | null {
  const subject = self.subject;
...
matchers[C_BACKSLASH] = function (self: InlineParser, pos: number, endpos: number): number | null {
  const subject = self.subject;
...
matchers[C_LESSTHAN] = function (self: InlineParser, pos: number, endpos: number): number | null {
  const subject = self.subject;
...
matchers[C_TILDE] = betweenMatched('~', 'subscript', 'str', alwaysTrue);

matchers[C_HAT] = betweenMatched('^', 'superscript', 'str', alwaysTrue);

matchers[C_UNDERSCORE] = betweenMatched('_', 'emph', 'str', alwaysTrue);

matchers[C_ASTERISK] = betweenMatched('*', 'strong', 'str', alwaysTrue);

matchers[C_EQUALS] = betweenMatched("=", "mark", "str", hasBrace);

matchers[C_SINGLE_QUOTE] = betweenMatched("'", "single_quoted", "right_single_quote",
...
matchers[C_DOUBLE_QUOTE] = betweenMatched('"', "double_quoted", "left_double_quote", alwaysTrue);

matchers[C_LEFT_BRACE] = function (self: InlineParser, pos: number, endpos: number): number | null {
...
matchers[C_COLON] = function (self: InlineParser, pos: number, endpos: number): number | null {
...
matchers[C_PERIOD] = function (self: InlineParser, pos: number, endpos: number): number | null {
...
matchers[C_LEFT_BRACKET] = function (self: InlineParser, pos: number, endpos: number): number {
...
matchers[C_RIGHT_BRACKET] = function (self: InlineParser, pos: number, endpos: number): number | null {
...
matchers[C_LEFT_PAREN] = function (self: InlineParser, pos: number, endpos: number): number | null {
  if (!self.destination) {
    return null;
  }
  self.addOpener("(", pos, pos, "str");
...
matchers[C_RIGHT_PAREN] = function (self: InlineParser, pos: number, endpos: number): number | null {
  if (!self.destination) {
    return null;
  }
...
matchers[C_HYPHEN] = function (self: InlineParser, pos: number, endpos: number): number | null {
  const subject = self.subject;
...
class InlineParser {
  ...
  addMatch(startpos: number, endpos: number, annot: string, matchIndex?: number, data?: Record<string, any>): void {
    const match: Event = { startpos: startpos, endpos: endpos, annot: annot };
    if (data) {
      match.data = data;
    }
